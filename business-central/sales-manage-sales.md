...
Puede corregir o cancelar fácilmente una factura de venta registrada antes de que el cliente la pague. Por ejemplo, para corregir un error de escritura o si el cliente solicita un cambio temprano en el proceso de pedido. Si la factura de venta registrada se ha pagado, deberá crear una nota de crédito de venta o un pedido de devolución de venta para revertir la venta.
...
## <a name="quote-to-order-to-sales-invoice"></a>De cotización a pedido a factura de venta

La siguiente tabla describe cómo usar procesos de venta simples.

|Para...| Vea |
|---|---|
| Crea una cotización de venta para ofrecer productos según términos negociables antes de convertir la cotización en una factura de venta. |[Crear cotizaciones de venta](sales-how-make-offers.md) |
...
|Facturar a un cliente una vez por varios envíos, combinándolos en una única factura.|[Combinar envíos en una factura única](sales-how-to-combine-shipments-on-a-single-invoice.md)|
|Vender elementos de ensamblado que no están disponibles actualmente mediante una orden de ensamblado vinculada. El pedido de ensamblado puede suministrar la cantidad total o parcial del pedido de venta.|[Vender artículos ensamblados para pedido](assembly-how-to-sell-items-assembled-to-order.md)|
...
| Aprenda cómo rastrear un paquete a partir de una remisión de venta registrada. | [Seguir paquetes](sales-how-track-packages.md) |
...
| Crea una nota de crédito de venta para revertir una factura de venta registrada específica para reflejar qué productos devuelve el cliente y el importe del reembolso. |[Procesar devoluciones de ventas o cancelaciones](sales-how-process-sales-returns-cancellations.md) |
