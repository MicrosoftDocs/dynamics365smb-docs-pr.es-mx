...
description: Este artículo ofrece una descripción general de las diferentes formas de recibir productos en un almacén, por ejemplo, productos con un pedido de compra o productos con una recepción de almacén.
author: SorenGP
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
3. Escriba la cantidad recibida en el campo **Cantidad a recibir**.
   ...
   > [!NOTE]
   > Si la cantidad recibida es mayor que la ordenada en la orden de compra, según el campo **Cantidad**, y el proveedor se ha configurado para permitir recepciones en exceso, entonces use el campo **Recepción en exceso** para administrar la cantidad en exceso. Obtenga más información en la sección [Para recibir más artículos de los solicitados](warehouse-how-receive-items.md#receive-more-items-than-ordered).
...
## <a name="receive-items-with-a-warehouse-receipt"></a>Recibir productos con recepción de almacén
...
3. Seleccione la acción **Tomar doc. origen**. Se abre la página **Documentos origen**.
    ...
    Desde una recepción de almacén nueva o abierta, puede utilizar la página **Filtros para traer docs. orig.** para recuperar las líneas del documento de origen lanzado que definen los productos para recibir o enviar.
    ...
    Todas las líneas del documento de origen lanzado que cumplan los criterios del filtro se insertarán en la página **Recep. almacén** en la que se activó la función del filtro. 

    Las combinaciones de filtros que defina se guardan en la página **Filtros para Tomar docs. orig.** hasta la próxima vez que las necesite. Puede crear un número ilimitado de combinaciones de filtros. Puede modificar los criterios en cualquier momento eligiendo la acción **Modificar**.
...
Cuando reciba más productos de los que ha ordenado, es posible que desee recibirlos en lugar de cancelar la recepción. Por ejemplo, puede ser más barato mantener el exceso en su inventario que devolverlos o su proveedor puede ofrecerle un descuento por conservarlos.
...
El código de recepción en exceso se asigna al artículo. Cualquier orden de compra o recepción de almacén para el artículo ahora permite recibir más que la cantidad solicitada de acuerdo con el porcentaje de tolerancia de recepción en exceso especificado.
