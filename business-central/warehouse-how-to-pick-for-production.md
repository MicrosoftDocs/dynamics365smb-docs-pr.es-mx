...
En configuraciones básicas de almacén en las que se requiere el proceso de picking pero no el de envío, también puede usar la página **Picking inventario** para organizar y registrar el picking de componentes con el método de baja establecido en *Manual*. Cuando registra un picking de inventario para una operación interna, como la producción, el consumo de los componentes seleccionados se registra al mismo tiempo. Como alternativa, puede usar **Movimiento inventario** con referencia a un documento de origen para llevar los componentes con el método de baja establecido en *Manual*, *Pick + Adelante*, *Pick + Atrás* a las órdenes de producción.
...
Las opciones **Movimiento inventario** con referencias al documento de origen y **Picking almacén** no se pueden usar para realizar el picking de componentes con métodos de baja *Adelante* y *Atrás*. No se puede usar **Picking inventario** para realizar el picking de componentes con cualquier método de baja, sino *Manual*. Para manipular los componentes restantes, use **Movimiento inventario** sin referencia a un documento de origen. Para obtener más información, consulte [Mover componentes a un área de operaciones en las configuraciones básicas de almacén](warehouse-how-to-move-components-to-an-operation-area-in-basic-warehousing.md).
...
Como alternativa, puede usar la opción **Movimiento inventario** con referencia al documento de origen para mover productos entre ubicaciones. Deberá registrar el consumo por separado. Para obtener más información, consulte [Registrar consumibles de producción por lotes](production-how-to-post-consumption.md)
...
En configuraciones básicas de almacén, también puede realizar el picking para las órdenes de ensamblado con la página **Movimiento inventario**. 

En configuraciones básicas de almacén en las que la ubicación requiere el proceso de picking pero no el proceso de envío, la página **Picking inventario** también se usa para realizar el picking, ensamblar y enviar órdenes de venta en las que los productos deben ensamblarse antes de que puedan enviarse. Para obtener más información, consulte [Gestión de productos de ensamblar para pedido con picking de inventario](warehouse-how-to-pick-for-production.md#handling-assemble-to-order-items-with-inventory-picks).  
...
Puede configurar [!INCLUDE[prod_short](includes/prod_short.md)] para que cree automáticamente un movimiento de inventario cuando se crea el picking de inventario para el producto de ensamblado. Para habilitarlo, debe seleccionar el campo **Crear movimientos automáticamente** en la página **Conf. ensamblado**. Para obtener más información, consulte [Mover componentes a un área de operaciones en el almacenamiento básico](warehouse-how-to-move-components-to-an-operation-area-in-basic-warehousing.md).
