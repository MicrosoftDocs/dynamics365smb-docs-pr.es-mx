...
La contabilidad se configura con la divisa local ($), pero también se puede configurar para usarla en otra divisa a la que se asigna un tipo de cambio. Si designa una segunda divisa como una divisa de informes adicional (DA), [!INCLUDE[prod_short](includes/prod_short.md)] registrará automáticamente los importes tanto en divisa local ($) como en divisa adicional (DA) en cada movimiento C/G y en otras movimientos, como los de IVA.

> [!Warning]
> No debería utilizar la función de divisa adicional como base para la conversión de estados de cuenta financieros a menos que comprenda sus limitaciones. No puede traducir estados de cuenta financieros de las subsidiarias extranjeras como parte de la consolidación de una empresa. La divisa adicional (DA) solo puede utilizarse para elaborar informes en otra divisa, como si esa divisa fuese la divisa local ($) de la empresa.
>
> Por ejemplo, tiene una gran cantidad de cuentas por cobrar en libras esterlinas (GBP) y ha configurado su DA como GBP. En este escenario, los importes en las cuentas por cobrar que utilizan libras esterlinas no se ajustan por las ganancias / pérdidas por cambio de divisa en la DA, solo los importes en las cuentas por cobrar que estén en otras divisas. Eso significa que si usa DA para notificar de sus estados de cuenta financieros, podría producirse saldos pendientes de cuentas por cobrar subestimados o exagerados.
...
|Campo|Descripción|  
|---------------------------------|---------------------------------------|  
|**Cta. aj. pos. CG realizados**|La cuenta de contabilidad general en la que desea que el sistema registre las ganancias de tipo de cambio de los ajustes de divisa entre divisa local ($) y divisa adicional (DA).|  
|**Cta. aj. neg. CG realizados**|La cuenta de contabilidad general en la que desea que el sistema registre las pérdidas de tipo de cambio de los ajustes de divisa entre divisa local ($) y divisa adicional (DA).|  
|**Cta. aj. residual pos.**|La cuenta de contabilidad general en la que se registrarán los importes residuales que sean ganancias si se realizan registros en el área de contabilidad general de la aplicación en divisa local ($) y divisa adicional (DA).|  
|**Cta. aj. residual neg.**|La cuenta de contabilidad general en la que se registrarán los importes residuales que sean pérdidas si se realizan registros en el área de contabilidad general de la aplicación en divisa local ($) y divisa adicional (DA).|

> [!NOTE]  
> Se pueden generar importes residuales cuando [!INCLUDE[prod_short](includes/prod_short.md)] redondea importes del debe y el haber que se han convertido de divisa local ($) a divisa adicional (DA).  

Para cada cuenta de contabilidad general, debe especificar cómo se ajustarán los importes de contabilidad general de esa cuenta en función de las fluctuaciones del tipo de cambio entre la divisa local ($) y la divisa adicional (DA).  
...
3. En la página **Ficha cuenta**, seleccione el método correspondiente en el campo **Ajuste tipo cambio**.  
    ...
    Si realiza registros en DA, en el campo **Ajuste tipo cambio**, especifique cómo se ajusta esta cuenta de contabilidad general en función de las fluctuaciones del tipo de cambio entre la divisa local ($) y la divisa adicional (DA). La siguiente tabla describe las opciones.  

    |Campo|Descripción|  
    |----------------------------------|---------------------------------------|  
    |**Sin ajuste**|No se realiza ningún ajuste del tipo de cambio en la cuenta. Esta configuración es la opción predeterminada.<br /><br /> **NOTA:** Seleccione esta opción si el tipo de cambio entre divisa local ($) y divisa adicional (DA) siempre es fijo.|  
...
3. Si realiza registros en DA, en el campo **Tipo cambio ajuste IVA** puede especificar cómo se ajustarán las cuentas para el registro del IVA de la página **Config. grupos registro IVA** en función de las fluctuaciones del tipo de cambio entre la divisa local ($) y la divisa adicional (DA).  
...
5. El trabajo por lotes **Ajustar divisa de informe adicional** se abre.
    ...
    Este proceso convierte los importes de divisa local ($) de los movimientos existentes a la divisa adicional (DA). El proceso usa un tipo de cambio predeterminado, copiado del que es válido en la fecha de trabajo de la página **Tipos cambio divisa**. La conversión de divisa local ($) a divisa adicional (DA) puede producir importes residuales, que se registran en las cuentas de ganancias y pérdidas residuales especificadas en la página **Divisas**. La fecha de registro y el número de documento de estos movimientos son los mismos que los del movimiento de contabilidad original. Después de haber registrado todos los movimientos residuales, el proceso registra un movimiento de redondeo en la fecha de cierre de cada año cerrado en la cuenta de remanentes. Este registro garantiza que el saldo final de las cuentas de resultados de cada ejercicio cerrado sea 0 tanto en divisa local ($) como en divisa adicional (DA).
...
Después de ejecutar el trabajo por lotes, los importes de los siguientes movimientos existentes se encuentran tanto en divisa local ($) como en divisa adicional (DA).  
...
Además, los importes de todos los movimientos posteriores del mismo tipo se registrarán en la divisa local ($) y la divisa adicional (DA).  
