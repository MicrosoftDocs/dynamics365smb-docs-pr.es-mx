...
Puede incluir cuentas bancarias en las transacciones del diario general. Cada cuenta bancaria se vincula a una cuenta del catálogo de cuentas a través del grupo contable de la cuenta bancaria asignada. El uso de una cuenta bancaria en una transacción de pago crea automáticamente una entrada tanto en la cuenta bancaria como en la cuenta de contabilidad (G/L) conectada.  

Las cuentas bancarias funcionan de manera diferente dependiendo de si se especifica un código de divisa:

- Si no se especifica un código de divisa, todas las transacciones en la cuenta bancaria se realizan en la divisa local ($) de la empresa actual. Si realiza una transacción para la cuenta en otra divisa, los importes se registran en la cuenta de divisa local ($) según el tipo de cambio de la divisa correspondiente. Cualquier cheque que se emita desde esta cuenta debe estar en divisa local ($). Si la cuenta bancaria se utiliza en un diario, la línea del diario usa automáticamente el código de divisa en blanco.  
