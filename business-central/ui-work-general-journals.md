...
## <a name="use-journal-templates-and-batches"></a>Usar libros y secciones de diario

Existen varias plantillas de diario general. Cada libro diario se representa mediante una página específica con funciones particulares y los campos que se requieren para admitir estas funciones, como la página **Diario de conciliación de pagos** para procesar pagos bancarios y la página **Diario de pagos** para pagar a sus proveedores o reembolsar a sus empleados. Para obtener más información, vea [Realizar pagos](payables-make-payments.md) y [Conciliar los pagos de clientes con el diario de recepción de efectivo o de los movimientos de cliente](receivables-how-apply-sales-transactions-manually.md).
...
Un diario periódico es un diario general con campos específicos para administrar las transacciones que registre frecuentemente con pocos cambios o con ninguno. Por ejemplo, transacciones de gastos como alquiler, suscripciones, electricidad y calefacción. El uso de diarios periódicos le permite registrar importes fijos y variables, y especificar movimientos de reversión automática para el día posterior a la fecha de registro. Las claves de asignación le permiten dividir los movimientos periódicos entre varias cuentas. Para obtener más información, consulte [Asignación de importes de diario recurrentes a varias cuentas](#allocating-recurring-journal-amounts-to-several-accounts).
...
|S Saldo|El importe registrado en la cuenta de la línea se distribuirá entre las cuentas especificadas para la línea de la tabla Diario gen. distribución. El saldo de la cuenta se establecerá a cero. No olvide rellenar el campo **% Distribución** en la página **Asignaciones**. Para obtener más información, consulte [Asignación de importes de diario recurrentes a varias cuentas](#allocating-recurring-journal-amounts-to-several-accounts).|
...
|SD Saldo por dimensión|La línea de diario asigna los costos según el saldo de una cuenta de contabilidad general por dimensión. Se le pedirá que configure los filtros de dimensión que se utilizarán para calcular el saldo de la cuenta de contabilidad general de origen por dimensión desde la que desea asignar los costos. Alternativamente, elija más tarde la acción **Establecer filtros de dimensión**.|
|Contraasiento saldo RBD por dimensión|La línea de diario asigna los costos según el contraasiento una cuenta de contabilidad general por dimensión. Se le pedirá que configure los filtros de dimensión que se utilizarán para calcular el saldo de la cuenta de contabilidad general de origen por dimensión desde la que desea asignar los costos. También puede elegir más tarde la acción **Establecer filtros de dimensión**.|
...
Si desea registrar un movimiento el último día de cada mes, haga una de las cosas siguientes:
...
Las acumulaciones generalmente se registran con los métodos periódicos **Fijo**, **Variable** o **Saldo** en la línea del diario. La fecha de registro del importe contabilizado en la cuenta en la línea de diario se calcula utilizando la frecuencia periódica. La fecha de registro del movimiento de contrapartida se calcula utilizando el campo **Cálculo de la fecha de reversión**, como sigue:
...
7. Para guardar los valores en el campo **Precio unitario** del diario de productos estándar, elija el campo **Guardar precio unitario**.
...
4. En la página **Diarios de productos estándar**, seleccione el diario de productos estándar a utilizar y, a continuación, elija **Aceptar**.
    ...
    Si no activa la opción **Guardar precio unitario**, el campo **Precio unitario** en las líneas agregadas desde el diario estándar contienen el valor del campo **Costo unitario** en la ficha del artículo.

    > [!NOTE]  
    > Si activó las opciones **Guardar precio unitario** o **Guardar cantidad**, asegúrese de que los nuevos valores sean correctos antes de registrar el diario de productos. <!--need to say where and when these were turned on-->
...
5. Seleccione las líneas del diario de productos que desea ajustar y, a continuación, elija la acción **Volver a calcular precio unitario**. Esta acción actualizará el campo Precio unitario con el costo unitario actual del producto.
...
[Revaluar inventario en el diario de revaluación](inventory-how-revalue-inventory.md)  
