...
Los grupos contables asignan entidades a cuentas de contabilidad. Algunos ejemplos de entidades son clientes, proveedores, artículos, recursos y documentos de compra y venta. Los grupos contables ahorran tiempo y ayudan a evitar errores al registrar las transacciones. Los valores de transacción se envían a las cuentas especificadas en el grupo contable de dicha entidad. El único requisito es que tenga un catálogo de cuentas. Para obtener más información, vea [Configuración del plan de cuentas](finance-setup-chart-accounts.md).  

Los grupos contables tienen tres divisiones:  

* General

  Defina a quién vende y de quién compra, y lo que vende y compra. También puede combinar grupos para especificar aspectos como las cuentas de resultados para registrar, o usar grupos para filtrar informes.  
* Específico
...
|Escriba | Description |
| --- | --- |
| Grupos registro cliente |Defina las cuentas que usará al enviar transacciones de cobros. Si usa un inventario con cobros, las cuentas en las que se registran las líneas de orden de venta están determinadas por el grupo contable o de negocio general asignado al cliente y el grupo contable de producto general asignado al producto del inventario. Consulte *Grupos contables negocio* y *Grupos contables de producto general* en la sección [Grupos contables generales](#general-posting-groups). Configure estos grupos contables en la página **Grupos contables clientes**. |
...
La configuración determina cuándo se realiza el registro. Por ejemplo, el tiempo se ve afectado por el momento en que realiza actividades periódicas, como registrar el costo de inventario o ajustar los movimientos de producto de costo.
...
Cuando algo anda mal, [!INCLUDE[prod_short](includes/prod_short.md)] mostrará la página **Mensajes de error**. La página **Mensajes de error** puede facilitar la identificación y resolución del problema. La página ofrece una descripción del error que señala la configuración del grupo de publicación que necesita atención. Por ejemplo, el mensaje podría ser "Falta una configuración de registro general en la cuenta de anticipo de ventas". También hay un enlace para abrir la página que es la fuente del problema para que pueda resolverlo rápidamente.  
