...
> [!TIP]
> Si suele planificar la demanda en diferentes almacenes, le recomendamos que use la funcionalidad Unidades de almacenamiento.
...
1. ¿Hay una marca de verificación en el campo **Almacén obligatorio** en la página **Configuración de inventario**?  
...
3. ¿El campo **Componentes en el almacén** en la página **Configuración fabricación** contiene el código de almacén solicitado?  
    ...
    El producto se planifica según estos parámetros: Política reorden = *Lote a lote*, Incluir inventario = *Sí*, todos los demás parámetros de planificación = vacíos. (Los productos que utilizan la política de reorden  *Pedido* continúan utilizando  *Pedido* así como los demás ajustes.)  
...
> [!TIP]
> El campo **Almacenes obligatorios** de la página **Configuración de inventario** y el campo **Componentes en almacén** de la página Configuración de fabricación son muy importantes para determinar cómo el sistema de planificación maneja las líneas de demanda con o sin códigos de almacén.
>
...
Otro campo importante es el campo **Cantidad máxima de pedido** en la ficha **Producto**. Especifica una cantidad máxima permitida para una propuesta de orden de producto y se utiliza si el producto se entrega en una unidad de transporte fija, como un contenedor, que, por ejemplo, desea que se utilice por completo. Una vez detectada la necesidad de reposición y ajustado el tamaño de lote para cubrir la política de reorden especificada, se disminuirá la cantidad, si es necesario, para cubrir la cantidad máxima de orden que ha definido para el producto. Si se mantienen las demandas adicionales, se calcularán nuevos pedidos para cubrirlas. Por lo general, este campo se usa con una política de fabricación Fab-contra-stock.  
...
El producto se planifica según esta lógica: Política reorden = *Lote a lote* (*Orden* sigue siendo *Orden)*, Incluir inventario = *Sí*, todos los demás parámetros de planificación = vacíos.  
...
El producto se planifica en función de los siguientes parámetros: Política reorden = *Lote a lote* (*Orden* sigue siendo *Orden*), Incluir inventario = *Sí*, todos los demás parámetros de planificación = vacíos.  
