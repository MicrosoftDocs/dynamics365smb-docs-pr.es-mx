...
Por ejemplo, un vendedor en [!INCLUDE[crm_md](includes/crm_md.md)] puede utilizar las listas de precios de [!INCLUDE[prod_short](includes/prod_short.md)] cuando se crea un pedido de venta. Cuando agrega el producto a la línea de la orden de venta en [!INCLUDE[crm_md](includes/crm_md.md)], puede ver el nivel de inventario (disponibilidad) del producto desde [!INCLUDE[prod_short](includes/prod_short.md)].
...
## <a name="couple-records"></a>Emparejamiento de registros
...
## <a name="fully-synchronize-records"></a>Sincronización completa de los registros
...
## <a name="handle-sales-order-data"></a>Manejar datos de pedidos de venta
...
## <a name="handle-sales-quotes-data"></a>Manejar datos de cotizaciones de venta
Las cotizaciones de venta que se activan en [!INCLUDE[crm_md](includes/crm_md.md)] se transferirán a [!INCLUDE[prod_short](includes/prod_short.md)] si selecciona la casilla **Procesar automáticamente cotizaciones** en la página **Configuración de conexión de Microsoft Dynamics 365**.
Como alternativa, puede convertir manualmente las cotizaciones de venta activadas desde [!INCLUDE[crm_md](includes/crm_md.md)] utilizando la acción **Procesar en [!INCLUDE[prod_short](includes/prod_short.md)]** en la página **Cotizaciones venta - Dynamics 365 Sales**.
En dichas cotizaciones de venta, el campo **Nombre** de la cotización original se transfiere y se asigna al campo **Número de documento externo** del pedido de venta en [!INCLUDE[prod_short](includes/prod_short.md)]. También se transfiere el campo **En vigor hasta** de la cotización y se asigna al campo **Cotización válida hasta** en la cotización de venta en [!INCLUDE[prod_short](includes/prod_short.md)].  

Las cotizaciones de venta pasan por muchas revisiones mientras se están finalizando. El procesamiento manual y automático de las cotizaciones de venta en [!INCLUDE[prod_short](includes/prod_short.md)] garantiza que las versiones anteriores de las cotizaciones de venta se archiven antes de procesar nuevas revisiones de cotizaciones de venta desde [!INCLUDE[crm_md](includes/crm_md.md)].

Cuando elige **Procesar** en [!INCLUDE[prod_short](includes/prod_short.md)] para una cotización que está en estado **Ganada**, se crea un pedido de venta en [!INCLUDE[prod_short](includes/prod_short.md)] solo si se envía un pedido de venta correspondiente en [!INCLUDE[crm_md](includes/crm_md.md)]. De lo contrario, la cotización solo se publica en [!INCLUDE[prod_short](includes/prod_short.md)]. Si se envía un pedido de venta correspondiente en [!INCLUDE[crm_md](includes/crm_md.md)] más tarde y se crea un pedido de venta a partir de este, el campo **N.º de cotización** se actualiza en el pedido de venta y se archiva la cotización.

## <a name="handle-posted-sales-invoices-customer-payments-and-statistics"></a>Gestión de histórico de facturas de venta, pagos de cliente y estadísticas
...
## <a name="handle-sales-prices"></a>Manejo de precios de venta
> [!NOTE]
